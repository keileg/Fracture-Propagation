...
plot.trisurf2d(
    fig, grid, p, t, eleind=0, nodind=0, line=nodcra, point=tipcra, parame=[]
)
plt.show()
plt.pause(0.0001)
...
while proste < 35:
    ...
    disp, consla, conmas, traction = contact.contactsolver(
        p6, t6, material, qpe, lefhs, righs, dirdof, dirval, slapoi, maspoi, gap, dc
    )

    Gi, ki, keq, craang = solution.SIF(
        tipava, p6, t6, numtip, disp, young, poisson, k, qpe
    )
    if max(abs(keq)) >= KIC:
