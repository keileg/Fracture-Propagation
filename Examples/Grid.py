...
class ReMesh:
    ...
    def refinement(self, tipava, crapro, error, A0, lmin, option):
        ...
        xc = (
            self.p[self.t[:, 0], :] + self.p[self.t[:, 1], :] + self.p[self.t[:, 2], :]
        ) / 3
    ...
    def quaterelement(self, tipava, crapro, lmin):
        ...
        for i in range(numtip):
            if tipava[i] == True and crapro[i] == True:
                tip = self.tipcra[i]
                pvcr = self.nodcra[i]
                self.p, self.t, pb, pt, paround, indexqpe = self.tipmesh(
                    tip, pvcr, lmin
                )
    ...
    def adjustmesh(self, tipava, KIC, Gi, keq, lmin, iniang, gloang, craang):
        ...
        ladv[np.int32(np.where(abs(keq) >= KIC)[0])] = (
            lmin
            * (
                Gi[np.int32(np.where(abs(keq) >= KIC)[0])]
                / max(Gi[np.int32(np.where(abs(keq) >= KIC)[0])])
            )
            ** 0.35
        )
        consif = abs(keq) >= KIC
        conlen = ladv >= 0.5 * lmin
        ...
        for i in range(len(crapro)):
            if tipava[i] == True and crapro[i] == True:
                ...
                if min(discra) > 3.0 * max(ladv):
                    dis = np.sqrt(
                        (pa[:, 0] - tipnew[0]) ** 2 + (pa[:, 1] - tipnew[1]) ** 2
                    )
                    ind = np.argmin(dis)
                    local = aux.p2index(self.p, pa[ind, :], 0)
                    self.p[local, 0] = tipnew[0]
                    self.p[local, 1] = tipnew[1]
                    # draw = PlotModel.draw_model(t, p, 3, 1)
                    self.p, self.t, self.tipcra, self.moucra, self.nodcra = self.splitelement(
                        i, tipnew
                    )
                    ...
                    self.p, self.t, self.tipcra, self.moucra, self.nodcra = self.connectcrack(
                        i, self.nodcra[np.argmin(discra)]
                    )
                    tipava[i] = False
        return (
            self.p,
            ...
            for i in range(len(local)):
                ...
                for e in range(self.t.shape[0]):
                    ...
                    if ds <= np.finfo(float).eps * 1e5 and dv != 0:
                        ...
                        d01 = aux.p2segment(
                            pi.reshape(1, 2), self.p[self.t[e, [0, 1]], :]
                        )
                        d02 = aux.p2segment(
                            pi.reshape(1, 2), self.p[self.t[e, [0, 2]], :]
                        )
                        l01 = (
                            (self.p[self.t[e, 0], 0] - self.p[self.t[e, 1], 0]) ** 2
                            + (self.p[self.t[e, 0], 1] - self.p[self.t[e, 1], 1]) ** 2
                        )
                        l02 = (
                            (self.p[self.t[e, 0], 0] - self.p[self.t[e, 2], 0]) ** 2
                            + (self.p[self.t[e, 0], 1] - self.p[self.t[e, 2], 1]) ** 2
                        )
                        l12 = (
                            (self.p[self.t[e, 1], 0] - self.p[self.t[e, 2], 0]) ** 2
                            + (self.p[self.t[e, 1], 1] - self.p[self.t[e, 2], 1]) ** 2
                        )
                        if d01 <= np.finfo(float).eps * 1e5:
                            if l01 >= max(l02, l12):
                                ...
                                p02 = (
                                    self.p[self.t[e, 0], :] + self.p[self.t[e, 2], :]
                                ) / 2
                                pe = np.array(([[p02[0], p02[1]]]))
                                te = np.array(
                                    (
                                        [
                                            [local[i], numnod + cou, self.t[e, 0]],
                                ...
                                p12 = (
                                    self.p[self.t[e, 1], :] + self.p[self.t[e, 2], :]
                                ) / 2
                            ...
                            if l02 >= max(l01, l12):
                                ...
                                p01 = (
                                    self.p[self.t[e, 0], :] + self.p[self.t[e, 1], :]
                                ) / 2
                                pe = np.array(([[p01[0], p01[1]]]))
                                te = np.array(
                                    (
                                        [
                                            [local[i], self.t[e, 0], numnod + cou],
        ...
        if t0.shape[0] < self.t.shape[0]:
            ...
            for i in range(xc.shape[0]):
                ...
                for j in range(i + 1, xc.shape[0]):
                    xcj = xc[j, :]
                    if (
                        abs(xci[0] - xcj[0]) < np.finfo(float).eps * 1e5
                        and abs(xci[1] - xcj[1]) < np.finfo(float).eps * 1e5
                    ):
                        eledel.append(j)
    ...
    def tipmesh(self, tip, pvcr, lmin):
        ...
        if l1 > 1.8 * l2:
            ...
            elein = np.intersect1d(
                np.where(self.t == tipind)[0], np.where(self.t == P1Ind)[0]
            )
            id0 = np.setdiff1d(self.t[elein, :], [tipind, P1Ind])[0]
            tnew = np.array(([[id0, tipind, indnew], [id0, P1Ind, indnew]]))
            ...
            indnew = self.p.shape[0]
            elein = np.intersect1d(
                np.where(self.t == tipind)[0], np.where(self.t == P2Ind)[0]
            )
            id0 = np.setdiff1d(self.t[elein, :], [tipind, P2Ind])[0]
            tnew = np.array(([[id0, tipind, indnew], [id0, P2Ind, indnew]]))
            self.t = np.delete(self.t, elein, axis=0)
            self.t = np.append(self.t, tnew, axis=0)
            self.p = np.append(self.p, pnew, axis=0)
            P2Ind = indnew
        ...
        A1Ind = np.setdiff1d(
            np.intersect1d(NodArouP1, bcInd), np.array([P1Ind, tipind])
        )[0]
        NodArouP2 = self.t[np.where(self.t == P2Ind)[0], :]
        NodArouP2 = np.unique(NodArouP2)
        A2Ind = np.setdiff1d(
            np.intersect1d(NodArouP2, bcInd), np.array([P2Ind, tipind])
        )[0]
        IndOut, nodedelete, eledelete = self.regionremesh(
            tipind, np.max(np.array(([r, lmin]))), A1Ind, A2Ind
        )
        pv1 = p1[inOut[-1], :]
        ...
        Paround = p1[1:, :]
        id0 = np.intersect1d(
            np.where(tip[0] == self.p[:, 0])[0], np.where(tip[1] == self.p[:, 1])[0]
        )
        idb = np.intersect1d(
            np.where(pbel[0] == self.p[:, 0])[0], np.where(pbel[1] == self.p[:, 1])[0]
        )
        idu = np.intersect1d(
            np.where(pupp[0] == self.p[:, 0])[0], np.where(pupp[1] == self.p[:, 1])[0]
        )
        indexCQPE = np.array([idb, id0, idu])
        self.p[idb, 0] = pbel0[0]
        self.p[idb, 1] = pbel0[1]
        self.p[idu, 0] = pupp0[0]
        self.p[idu, 1] = pupp0[1]
    ...
    def regionremesh(self, tipind, r, A1Ind, A2Ind):
        ...
        tt = np.copy(self.t)
        p0 = self.p[tipind, :]
        nodescloset = np.where(
            np.sqrt((self.p[:, 0] - p0[0]) ** 2 + (self.p[:, 1] - p0[1]) ** 2)
            <= 1.2 * r
        )
        nodescloset = np.union1d(nodescloset, IndIn)

        elemecloset = np.where(
            np.array(
                [
                    aux.ismember(self.t[:, 0], nodescloset),
                    aux.ismember(self.t[:, 1], nodescloset),
                    aux.ismember(self.t[:, 2], nodescloset),
                ]
            )
            == 1
        )[1]
        ...
        Elei = np.array(
            [
                aux.ismember(self.t[:, 0], nodesaround),
    ...
    def fillgap(self, pv):
        ...
        while arepol > np.sum(areele):
            ...
            if ang1 > ang2:
                ...
                p13 = (pv[ti[0], :] + pv[ti[2], :]) / 2
                p13 = p13.reshape(1, 2)
                d13 = aux.p2segment(p13, pv)

                p23 = (pv[ti[1], :] + pv[ti[2], :]) / 2
                p23 = p23.reshape(1, 2)
    ...
    def smoothing(self, nodfix):
        ...
        for j in range(2):
            for i in range(len(nodche)):
                ...
                self.p[nodche[i], 0] = p0[0]
                self.p[nodche[i], 1] = p0[1]
                ...
                totare1 = sum(Ae)
                if totare1 > totare:
                    self.p[nodche[i], 0] = nodmov[0]
                    self.p[nodche[i], 1] = nodmov[1]
    ...
    def splitelement(self, tipi, tipnew):
        ...
        indout = self.t[elearo, :]
        indout = np.unique(indout)
        indout = np.setdiff1d(indout, [bb, bt, b1])
        indouti = np.copy(bb)
        A0 = (self.p[bb, :] + self.p[bt, :]) / 2
        A1 = self.p[b2, :]
        A2 = self.p[indout, :]
        d0 = (A1[0, 0] - A0[0, 0]) * (A2[:, 1] - A0[0, 1]) - (A1[0, 1] - A0[0, 1]) * (
            A2[:, 0] - A0[0, 0]
        )
        ...
        sumind = (
            np.array(aux.ismember(self.t[elearo, 0], indouti))
            + np.array(aux.ismember(self.t[elearo, 1], indouti))
            + np.array(aux.ismember(self.t[elearo, 2], indouti))
        )
        ...
        tipincra = np.intersect1d(
            np.where(pvcra[:, 0] == tip[0])[0], np.where(pvcra[:, 1] == tip[1])[0]
        )[0]
    ...
    def connectcrack(self, tipi, confac):
        ...
        for i in range(len(bc)):
            ...
            if dis < dis0:
                ...
                pnew = poi
        bar = np.concatenate(
            (self.t[:, [0, 1]], self.t[:, [0, 2]], self.t[:, [1, 2]]), axis=0
        )
        ...
        for i in range(bar.shape[0]):
            ...
            d1 = (pnew[0] - tip[0]) * (M1[1] - tip[1]) - (pnew[1] - tip[1]) * (
                M1[0] - tip[0]
            )
            d2 = (pnew[0] - tip[0]) * (M2[1] - tip[1]) - (pnew[1] - tip[1]) * (
                M2[0] - tip[0]
            )
            if (
                abs(d1) > np.finfo(float).eps * 1e5
                and abs(d2) > np.finfo(float).eps * 1e5
                and np.sign(d1) != np.sign(d2)
                and flag == 2
            ):
        ...
        for i in range(len(poi)):
            ...
            if len(O2) == 2:
                ...
                tipincra = np.intersect1d(
                    np.where(pvcra[:, 0] == O1[0])[0], np.where(pvcra[:, 1] == O1[1])[0]
                )[0]
                nodcranew = np.zeros(shape=(pvcra.shape[0] + 4, 2))
                nodcranew[:tipincra, :] = pvcra[:tipincra, :]
                ...
                nodcranew[tipincra + 5 : :, :] = pvcra[tipincra + 1 : :, :]
                self.tipcra[tipi] = O2
                self.moucra[tipi] = np.concatenate(
                    ([pbnew.reshape(1, 2), ptnew.reshape(1, 2)])
                )
                self.nodcra[tipi] = nodcranew
            else:
                O1 = self.tipcra[tipi]
