...
class AuxiFu:
    ...
    def p2index(self, p, pv, sort):
        ...
        if len(pv.shape) == 1:
            ...
            if sort == 1:
                dis = np.sqrt(
                    (p[bcInd, 0] - pv[0, 0]) ** 2 + (p[bcInd, 1] - pv[0, 1]) ** 2
                )
    ...
    def angle(self, p1, p0, p2):
        ...
        if l10 * l20 == 0:
            ...
            n2 = p10 / l10
            angle = np.arctan2(
                abs(n1[0] * n2[1] - n1[1] * n2[0]), n1[0] * n2[0] + n1[1] * n2[1]
            )
        return angle
    ...
    def inpolygon(self, p, pv):
        ...
        for i, x in enumerate(linex):
            ...
            for p0, p1 in lines():
                if ((p0[1] > y) != (p1[1] > y)) and (
                    x < ((p1[0] - p0[0]) * (y - p0[1]) / (p1[1] - p0[1]) + p0[0])
                ):
                    result = not result
            mask[i] = result
    ...
    def on_segment(self, p1, p2, p):
        return min(p1.x, p2.x) <= p.x <= max(p1.x, p2.x) and min(
            p1.y, p2.y
        ) <= p.y <= max(p1.y, p2.y)
    ...
    def projection(self, A, B, M):
        ...
        if dis1 < np.finfo(float).eps * 1e3:
            ...
            if dis < np.finfo(float).eps * 1e3:
                if (
                    abs(np.sqrt(sum((A - N) ** 2)) + np.sqrt(sum((B - N) ** 2)) - AB)
